...
8. 因果、结果、记账与停止点：列出局部因果、直接结果、全部记账依据和交互停止位置。
...
- 随后必须输出且只能输出一个 <state_update>严格 JSON</state_update>，内容只能是 {"changed":true} 或 {"changed":false}，不得添加属性、代码围栏或其他字段。
...
export const MAIN_SINGLE_CALL_DELIVERY_REMINDER = `【单次交付硬约束】
本回合只有当前这一次生成机会。输出前先为完整结构化尾部预留空间：必要业务标签（可为零个） -> 唯一 <state_update> -> 唯一 <memory> -> 唯一有效 <shinobi_daily>；若另有 <image_contract>，只能放在日报之后。
${MAIN_REASONING_CHECKLIST_PROMPT}
可见正文以 900-1500 个汉字为目标。接近输出上限时优先缩短正文中的非必要描写，不得省略、合并或截断固定八项和结构化尾部，也不得在 </shinobi_daily> 闭合前结束回复。
最终发送前逐项确认：正文中的每个移动、消耗、获得、伤势、任务、关系、战斗和事件结果都有可执行标签；确无变化才写 {"changed":false}。`;
