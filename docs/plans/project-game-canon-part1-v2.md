...
当前日期没有剧情时，运行时发送完整的最近一个未来剧情日，明确当前日期、目标日期和相距天数。该剧情日进入普通 `CURRENT_PLOT` 上下文，可以在当前分支提前引用、推进、改写和结算；提前使用剧情不会自动改变游戏日期。
...
应用层会拒绝不存在的项目正史 ID、非法状态和非法延期日期，但允许提前结算已存在的未来 `DAY/SCN/EV`。

## 权威与知识边界

运行时事实优先级：当前状态、开局契约和已发生记忆 > 项目世界书 > 项目正史时间线 > 忍术资料 > 模型预训练知识。

项目正史日期服务游戏因果，不代表漫画明示绝对日期。离屏场景和最近未来剧情日都属于可执行的普通剧情上下文，但普通角色只能知道其亲历、被告知、公开可查或合理推断的部分。
